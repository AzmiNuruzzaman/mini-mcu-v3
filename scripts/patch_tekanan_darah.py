...
def column_exists_sqlite(cursor):
    ...
    for row in rows:
        ...
        if len(row) >= 2 and str(row[1]).lower() == 'tekanan_darah':
            return True
    return False
...
def patch():
    ...
    with connection.cursor() as cursor:
        try:
            ...
            if exists:
                if vendor == 'postgresql':
                    cursor.execute("ALTER TABLE checkups ALTER COLUMN tekanan_darah TYPE VARCHAR(50);")
                    print("[DONE] Ensured 'tekanan_darah' is VARCHAR(50) on Postgres.")
                elif vendor == 'mysql':
                    cursor.execute("ALTER TABLE checkups MODIFY COLUMN tekanan_darah VARCHAR(50) NULL;")
                    print("[DONE] Ensured 'tekanan_darah' is VARCHAR(50) on MySQL.")
                elif vendor == 'sqlite':
            ...
            if vendor == 'postgresql':
                cursor.execute("ALTER TABLE checkups ADD COLUMN tekanan_darah VARCHAR(50);")
            elif vendor == 'mysql':
                cursor.execute("ALTER TABLE checkups ADD COLUMN tekanan_darah VARCHAR(50) NULL;")
            elif vendor == 'sqlite':
                cursor.execute("ALTER TABLE checkups ADD COLUMN tekanan_darah TEXT;")

            print("[DONE] Added column 'tekanan_darah' to table 'checkups' with preferred type.")
