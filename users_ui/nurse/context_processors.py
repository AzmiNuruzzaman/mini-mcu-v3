...
def nurse_menu(request):
    """Context processor to provide nurse menu items to all nurse views."""
    if not request.path.startswith('/nurse/'):
        return {}

    # Get first employee UID for Edit Karyawan / QR menu
    ...
    menu_items = [
        ...
        {
            'name': 'QR Codes',
            'url': reverse('nurse:qr_detail', kwargs={'uid': default_uid}) if default_uid else '#',
            'icon': 'qrcode',
            'key': 'qr'
        },
        {
            'name': 'Upload / Export Data',
            'url': reverse('nurse:upload_export'),
            'icon': 'upload',
            'key': 'upload_export'
        },
        {
    ...
    active_key = 'dashboard'  # default key

    path_prefixes = [
        ('/nurse/qr', 'qr'),
        ('/nurse/upload-export', 'upload_export'),
        ('/nurse/karyawan', 'edit_karyawan'),
        ('/nurse', 'dashboard'),
    ]
    ...
    # Derive human-readable label for display
    active_label = next((item['name'] for item in menu_items if item['key'] == active_key), 'Dashboard')

    # Avatar URL and role title for header/sidebar
    from django.conf import settings
    import os
    username = request.session.get('username') or getattr(getattr(request, 'user', None), 'username', None)
    avatar_url = None
    if username:
        avatar_dir = os.path.join(settings.MEDIA_ROOT, 'avatars', 'nurse')
        if os.path.isdir(avatar_dir):
            for ext in ['jpg', 'jpeg', 'png', 'webp', 'gif']:
                candidate = os.path.join(avatar_dir, f"{username}.{ext}")
                if os.path.exists(candidate):
                    avatar_url = f"{settings.MEDIA_URL}avatars/nurse/{username}.{ext}"
                    break
    role_title = 'Nurse'
    avatar_upload_url = None
    try:
        avatar_upload_url = reverse('nurse:upload_avatar')
    except Exception:
        avatar_upload_url = None

    # removed dynamic greeting block
...
def manager_menu(request):
    ...
    # Check specific URL path prefixes first, then fall back to '/manager'
    path_prefixes = [
